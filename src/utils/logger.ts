...
function shouldLog(level: keyof typeof LEVELS): boolean {
  const cfg = getConfig();
  return LEVELS[level] >= LEVELS[cfg.LOG_LEVEL];
}
...
export const logger = {
  debug(tag: string, msg: string) {
    if (shouldLog("debug")) console.error(fmt("debug", tag, msg));
  },
  info(tag: string, msg: string) {
    if (shouldLog("info")) console.error(fmt("info", tag, msg));
  },
  warn(tag: string, msg: string) {
    if (shouldLog("warn")) console.error(fmt("warn", tag, msg));
  },
