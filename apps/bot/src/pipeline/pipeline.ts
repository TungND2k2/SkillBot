...
export async function runPipeline(input: PipelineInput): Promise<PipelineOutput> {
  ...
  if (chatter) {
    ...
    headBlock.push(
      ...
      "",
      `## Nhắc tới 1 bản ghi (đơn hàng, khách, vải...)`,
      `Tool list/get/create/update đã trả sẵn link trong kết quả — LUÔN dùng link đó khi nhắc tới bản ghi, KHÔNG hiện ID thô (chuỗi hex dài) cho user.`,
      "",
      `## Tạo đơn hàng qua link web (không cần token)`,
      `Khi user nói "tạo đơn hàng", "gửi link tạo đơn", hoặc muốn tự điền/upload hoá đơn+đề bài trên web thay vì nhắn từng field:`,
      `→ Gọi ngay \`create_order_intake_link({ chatId: ${chatter.chatId} })\` — KHÔNG hỏi lại chatId. Gửi thẳng link nhận được cho user. Bot sẽ tự báo lại đúng chat này khi form được nộp xong.`,
      `Nếu user muốn tạo đơn NGAY qua chat (đọc từng field cho bạn nghe) thay vì qua link → vẫn dùng \`create_orders\` như bình thường.`,
      "",
      `CẤM hỏi:`,
